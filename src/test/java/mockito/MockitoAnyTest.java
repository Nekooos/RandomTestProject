...
public class MockitoAnyTest {
    ...
    public void anyTest() {
        ...
        Assertions.assertTrue(mockitoAny.isSomething("A", 1, "A"));
        Assertions.assertTrue(mockitoAny.isSomething("B", 10, new Object()));

        Mockito.when(mockitoAny.returnSomeInt(anyBoolean(), anyList())).thenReturn(2);
        Assertions.assertEquals(2, mockitoAny.returnSomeInt(true, new ArrayList<>()));

        Mockito.when(mockitoAny.returnSomeIntByte(any(byte[].class), any(String[].class), anyInt())).thenReturn(3);
        Assertions.assertEquals(3, mockitoAny.returnSomeIntByte(new byte[] {2}, new String[] {"derp"}, 3));
    }
}
